...
create or replace function public.fetch_poem_comments_drawer_page(
  target_poem_id uuid,
...
as $$
  ...
  reaction_rollup as (
    select
      t.id as comment_id,
      count(cr.user_id)::bigint as like_count,
      coalesce(bool_or(cr.user_id = auth.uid()), false) as liked_by_viewer
    from thread t
    left join public.comment_reactions cr on cr.comment_id = t.id
    group by t.id
  )
  select
    t.id,
    ...
    t.created_at,
    t.parent_comment_id,
    coalesce(rr.like_count, 0)::bigint as like_count,
    coalesce(rr.liked_by_viewer, false) as liked_by_viewer,
    pm.comment_count,
    cm.next_cursor_created_at,
    cm.next_cursor_id
  from thread t
  left join reaction_rollup rr on rr.comment_id = t.id
